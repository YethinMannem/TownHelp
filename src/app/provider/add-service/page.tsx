import { getServiceCategories } from '@/app/actions/booking'
...
export default async function AddServicePage() {
  ...
  const existingCategoryIds = profile.services?.map((s: any) => s.category?.id) || []
  const availableCategories = categories.filter(
    (c: any) => !existingCategoryIds.includes(c.id)
  )
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <p className="text-gray-600 mb-6">
          What service do you want to offer, {profile.display_name}?
        </p>
          ...
          <div className="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            {profile.services.map((s: any) => (
              <span key={s.id} className="inline-block text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded mr-2 mb-1">
                {s.category?.name} — ₹{s.custom_rate || profile.base_rate}/{s.rate_type?.toLowerCase() || 'hr'}
              </span>
            ))}
          </div>
        )}
